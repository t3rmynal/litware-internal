...
## ⚠️ Still in development. Some features are unfinished.
...
## Меню на Electron/TSX (опционально)

Текущее меню — **ImGui** внутри процесса игры: один процесс, нет задержки ввода, всё в одной DLL.

**Можно ли сделать меню на TSX/Electron?** Да, это возможно и даёт максимальную свободу в дизайне (React, анимации, сложные эффекты):

- **Архитектура:** отдельное окно Electron (прозрачное, поверх игры, always-on-top). DLL инжектируется как сейчас и открывает/слушает **IPC** (например Named Pipes или локальный HTTP/WebSocket).
- **Обмен настройками:** Electron-приложение пишет конфиг в файл или шлёт по пайпу; DLL периодически читает или подписывается на события. Либо DLL поднимает мини-сервер (localhost), фронт шлёт POST с настройками.
- **Плюсы:** любой UI (TSX, CSS, анимации, «летающие звёзды», сложные темы), привычный веб-стек, горячая перезагрузка при разработке.
- **Минусы:** два процесса, синхронизация конфига, позиционирование оверлея под разрешение игры, нужно скрывать окно при скриншотах/стриме.

Сейчас в проекте меню реализовано на ImGui с эффектами в стиле референса (сетка, звёзды, акцентное свечение). При желании можно вынести только UI в Electron и оставить в DLL логику чтения/применения настроек.
